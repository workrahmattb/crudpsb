  ...
  <body class="bg-gray-100 text-gray-800">
    <header class="bg-white shadow-md">
        <div class="container mx-auto flex items-center justify-between py-4 px-6">
          <nav class="bg-white dark:bg-gray-900 fixed w-full z-20 top-0 start-0 border-b border-gray-200 dark:border-gray-600">
              ...
              <a href="https://syafaaturrasul.com" class="flex items-center space-x-3 rtl:space-x-reverse">
                  ...
                  <span class="self-center font-semibold whitespace-nowrap dark:text-white"></span>
              </a>
      ...
      <section class="bg-green-500 text-white">
        <div class="container mx-auto flex flex-col items-center justify-center text-center py-20 px-6">
          <h1 class="text-4xl md:text-6xl font-bold mb-6"> Selamat Datang <br/> Pondok Pesantren Syafa'aturrasul</h1>
    ...
    <div class="container mx-auto p-4">
        <div class="flex justify-end mb-4">
            <a href="{{ route('products.create') }}" class="bg-gradient-to-r from-teal-400 to-teal-600 text-white font-semibold py-3 px-6 rounded-lg shadow-lg transform transition duration-300 hover:scale-105 hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-teal-300" hover:bg-gray-700">Create</a>
        </div>
        ...
        <div class="bg-white shadow-lg rounded-lg overflow-hidden">
            <div class="bg-green-800 p-6">
                <h3 class="text-white text-2xl font-semibold">Nama Pendaftar Santri Baru Pondok Pesantren Syafa'aturrasuls</h3>
            </div>
                ...
                <table class="min-w-full text-sm text-left text-gray-600 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-100 dark:bg-gray-800 dark:text-gray-400">
                        <tr>
                            ...
                            <th class="border px-4 py-3">Image2</th>
                            <th class="border px-4 py-3">Name</th>
                            <th class="border px-4 py-3">Sku</th>
                        ...
                        @if ($products->isNotEmpty())
                            @foreach ($products as $product)
                                <tr class="hover:bg-gray-50 transition-all duration-200">
                                    ...
                                    <td class="border px-4 py-3">{{ $product->sku }}</td>
                                    <td class="border px-4 py-3 text-green-600 font-semibold">${{ $product->price }}</td>
                                    <td class="border px-4 py-3">{{ \Carbon\Carbon::parse($product->created_at)->format('d M, Y') }}</td>
                                    <td class="border px-4 py-3 text-center">
                                        <div class="flex space-x-2 justify-center">
                                            <a href="{{ route('products.edit',$product->id) }}" class="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-500 transition-all duration-200">Edit</a>
                                            <a href="#" onclick="deleteProduct({{ $product->id }});" class="bg-red-600 text-white px-4 py-2 rounded-md hover:bg-red-500 transition-all duration-200">Delete</a>
                                        </div>
