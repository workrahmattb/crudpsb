        ...
        <div class="flex justify-center mt-8">
            <div class="w-full md:w-2/3">
                <div class="bg-white p-6 rounded-lg shadow-lg">
                    ...
                    <form enctype="multipart/form-data" action="{{ route('santris.update',$santri->id) }}" method="post">
                        @method('put')
                        @csrf
                        <div class="space-y-4">

                            <div>
                                <label for="nama" class="block text-lg font-medium">Nama</label>
                                <input value="{{ old('nama',$santri->nama) }}" type="text" class="w-full mt-2 p-3 border border-gray-300 rounded-lg @error('nama') border-red-500 @enderror" placeholder="Nama" name="nama">
                                @error('nama')
                            ...
                            <div>
                                ...
                                <input value="{{ old('sku',$santri->sku) }}" type="text" class="w-full mt-2 p-3 border border-gray-300 rounded-lg @error('sku') border-red-500 @enderror" placeholder="Sku" name="sku">
                                @error('sku')
                                    <p class="text-red-500 text-sm">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input value="{{ old('price',$santri->price) }}" type="text" class="w-full mt-2 p-3 border border-gray-300 rounded-lg @error('price') border-red-500 @enderror" placeholder="Price" name="price">
                                @error('price')
                                    <p class="text-red-500 text-sm">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <textarea placeholder="Description" class="w-full mt-2 p-3 border border-gray-300 rounded-lg" name="description" rows="5">{{ old('description',$santri->description) }}</textarea>
                            </div>
                            ...
                            <div>
                                ...
                                @if ($santri->image != "")
                                    <img class="w-1/2 my-3" src="{{ asset('storage/' . $santri->image) }}" alt="">
                                @endif
                            </div>
                            ...
                            <div>
                                ...
                                @if ($santri->image2 != "")
                                    <img class="w-1/2 my-3" src="{{ asset('storage/' . $santri->image2) }}" alt="">
                                @endif
                            </div>
